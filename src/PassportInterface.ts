...
export function getWithoutProvingUrl(zupassClientUrl: string, returnUrl: string, pcdType: string) {
...
export function constructZupassPcdAddRequestUrl(zupassClientUrl: string, returnUrl: string, pcd: SerializedPCD) {
...
export function constructZupassPcdProveAndAddRequestUrl<T extends PCDPackage = PCDPackage>(
    zupassClientUrl: string,
    returnUrl: string,
    pcdType: string,
    args: ArgsOf<T>,
...
export const getAnonTopicNullifier = (chatId: number, topicId: number): bigint => {
    return BigInt("0x" + sha256(JSON.stringify({ chatId, topicId })).substring(0, 16))
}
